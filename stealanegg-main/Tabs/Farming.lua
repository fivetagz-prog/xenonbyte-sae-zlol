...
local function UpdateOptionVisual(Name)
    ...
    if SelectedRarities[Name] then
        Option.BackgroundColor3 = Color3.fromRGB(210, 210, 210)
        Option.Text = "✓ " .. Name
    else
...
local function CreateDropdownOption(Name, Order)
    ...
    Option.MouseButton1Click:Connect(function()
        ...
        if _G.XENONBYTE_EggCheckPremium then
            local List = {}
            if SelectedRarities.Secret then table.insert(List, "Secret") end
            if SelectedRarities.Eternal then table.insert(List, "Eternal") end
            if SelectedRarities.Divine then table.insert(List, "Divine") end
            _G.XENONBYTE_EggCheckPremium.SetRarities(List)
        end

        -- ✅ Update FarmingManager
        if _G.XENONBYTE_FarmingManager then
            local List = {}
            if SelectedRarities.Secret then table.insert(List, "Secret") end
            if SelectedRarities.Eternal then table.insert(List, "Eternal") end
            if SelectedRarities.Divine then table.insert(List, "Divine") end
            _G.XENONBYTE_FarmingManager.SetRarities(List)
        end
...
local function ToggleFarm()
    if not _G.XENONBYTE_FarmingManager then
        warn("[XENONBYTE] FarmingManager not loaded!")
        return
    end
    ...
    if FarmEnabled then
        FarmButton.BackgroundColor3 = Color3.fromRGB(210, 210, 210)
        FarmStroke.Color = Color3.fromRGB(245, 245, 245)
        ...
        _G.XENONBYTE_FarmingManager.Enable()
    else
        FarmButton.BackgroundColor3 = Color3.fromRGB(28, 29, 39)
        FarmStroke.Color = Color3.fromRGB(200, 200, 220)
        _G.XENONBYTE_FarmingManager.Disable()
    end
end
...
task.spawn(function()
    task.wait(1)
    if _G.XENONBYTE_FarmingManager then
        local State = _G.XENONBYTE_FarmingManager.IsEnabled()
        FarmEnabled = State
...
task.spawn(function()
    while task.wait(1) do
        if _G.XENONBYTE_FarmingManager then
            local CurrentState = _G.XENONBYTE_FarmingManager.IsEnabled()
            ...
            if CurrentState ~= UIState then
                ...
                print("[XENONBYTE] Farming UI Sync | State: " .. tostring(CurrentState))
            end
        end
    end
end)
...
-- ==================================================
_G.XENONBYTE_RefreshFarmingUI = function()
    if _G.XENONBYTE_FarmingManager then
        local State = _G.XENONBYTE_FarmingManager.IsEnabled()
        FarmEnabled = State
        FarmCheck.Visible = State
        ...
        print("[XENONBYTE] Farming Tab UI Refreshed | State: " .. tostring(State))
    end
end

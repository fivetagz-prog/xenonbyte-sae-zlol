...
local SelectedMethod = _G.XENONBYTE_SelectedMethod or "TeleportFly"
...
local function CreateOption(Name, Order)
    ...
    Option.MouseButton1Click:Connect(function()
        ...
        _G.XENONBYTE_SelectedMethod = Name

        if _G.XENONBYTE_ConfigSystem then
            _G.XENONBYTE_ConfigSystem.Save()
        end

        print("[XENONBYTE] Method Teleport Selected: " .. Name)
    end)
...
if _G.XENONBYTE_SelectedMethod == nil then
    _G.XENONBYTE_SelectedMethod = "TeleportFly"
end
...
local InitialSpeed = _G.XENONBYTE_TeleportSpeed or 300
...
SpeedTextBox.FocusLost:Connect(function()
    ...
    if Value then
        ...
        _G.XENONBYTE_TeleportSpeed = Value

        if _G.XENONBYTE_TeleportSystem then
            _G.XENONBYTE_TeleportSystem.SetSpeed(Value)
        end
...
GodModeButton.BackgroundColor3 = Color3.fromRGB(210, 210, 210)
...
local function ShowNotification(Text)
    ...
    NotifyGui.Name = "XenonByteNotify"
...
FastClickButton.BackgroundColor3 = Color3.fromRGB(210, 210, 210)
...
FastClickButton.MouseButton1Click:Connect(function()
    if not _G.XENONBYTE_ManualFastClick then
        warn("[XENONBYTE] ManualFastClick feature not loaded")
        ShowNotification("Manual Fast Click Not Loaded")
        return
    end

    if _G.XENONBYTE_ManualFastClick.IsEnabled() then
        _G.XENONBYTE_ManualFastClick.Disable()
        ShowNotification("Manual Fast Click Stop")
    else
        _G.XENONBYTE_ManualFastClick.Enable()

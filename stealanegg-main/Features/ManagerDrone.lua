-- ==================================================
-- XENONBYTE HUB | FEATURE | Manager Drone
...
local function ForceStopAll()
    ...
    if _G.XENONBYTE_AttackDrone then
        pcall(function() _G.XENONBYTE_AttackDrone.Stop() end)
    end
    if _G.XENONBYTE_AFKSystem then
        pcall(function() _G.XENONBYTE_AFKSystem.Disable() end)
    end
end
...
local function SwitchAFKToAttack()
    ...
    if _G.XENONBYTE_AFKSystem then
        TreadmillPos = _G.XENONBYTE_AFKSystem.GetMyTreadmillPos()
    end

    if not TreadmillPos and _G.XENONBYTE_AFKSystem then
        local _, Treadmill = _G.XENONBYTE_AFKSystem.FindMyPlotAndTreadmill()
        if Treadmill then
    ...
    if not TreadmillPos then
        ...
        if _G.XENONBYTE_AFKSystem then
            _G.XENONBYTE_AFKSystem.Disable()
        end
        task.wait(0.5)
        if _G.XENONBYTE_AttackDrone then
            _G.XENONBYTE_AttackDrone.Start()
        end
        return
    end
    ...
    _G.XENONBYTE_AFKSystem.JumpOutTreadmill(TreadmillPos, function()
...
local function MainLoop()
    while ManagerEnabled do
        ...
        if EventNotActive then
            if _G.XENONBYTE_AttackDrone and _G.XENONBYTE_AttackDrone.IsEnabled() then
                print("[ManagerDrone] Event Not Active → Stop Attack")
                _G.XENONBYTE_AttackDrone.Stop()
            end

            if _G.XENONBYTE_AFKSystem and not _G.XENONBYTE_AFKSystem.IsEnabled() then
                print("[ManagerDrone] Event Not Active → AFK System")
                _G.XENONBYTE_AFKSystem.Enable()
            end
        -- ==================================================
        ...
        elseif EventActive then
            if _G.XENONBYTE_AFKSystem and _G.XENONBYTE_AFKSystem.IsEnabled() then
                print("[ManagerDrone] Event Active → Switch AFK to Attack")
                SwitchAFKToAttack()
            elseif _G.XENONBYTE_AttackDrone and not _G.XENONBYTE_AttackDrone.IsEnabled() then

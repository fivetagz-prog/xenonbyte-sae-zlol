-- ==================================================
-- XENONBYTE HUB | FEATURE | Bypass Anti Cheat
...
local function RunBypassAntiCheat()
    ...
    if not OldHumanoid then
        warn("[XENONBYTE] Humanoid not found")
        return
    end

    print("========================================")
    print("[XENONBYTE] START HUMANOID REPLACE")
    print("========================================")
    ...
    if not NewHumanoid then
        warn("[XENONBYTE] Failed to clone Humanoid")
        return
    end
    ...
    if not NewHumanoid.Parent then
        warn("[XENONBYTE] New Humanoid was removed")
        return
    end

    print("[XENONBYTE] New Humanoid:", NewHumanoid)

    --==================================================
    -- UPDATE CHARACTER SYSTEM
    --==================================================
    if _G.XENONBYTE_CharacterSystem then
        _G.XENONBYTE_CharacterSystem.CurrentHumanoid = NewHumanoid
        _G.XENONBYTE_CharacterSystem.CurrentRoot = Character:FindFirstChild("HumanoidRootPart")
        print("[XENONBYTE] CharacterSystem Updated with New Humanoid")
    end
    ...
    print("[XENONBYTE] Animator:", Animator)
    ...
    if Animate then
        print("[XENONBYTE] Restarting Animate...")
        pcall(function()
        ...
        print("[XENONBYTE] Animate restarted")
    end
    ...
    local function RefreshControls()
        ...
        if not PlayerScripts then
            warn("[XENONBYTE] PlayerScripts not found")
            return
        end
        local PlayerModule = PlayerScripts:FindFirstChild("PlayerModule")
        if not PlayerModule then
            warn("[XENONBYTE] PlayerModule not found")
            return
        end
        ...
        if not Success or not Module then
            warn("[XENONBYTE] Failed to require PlayerModule")
            return
        end
        local Controls
        pcall(function()
        ...
        if not Controls then
            warn("[XENONBYTE] Controls not found")
            return
        end
        pcall(function()
        ...
        task.wait()
        print("[XENONBYTE] Controls Humanoid:", Controls.humanoid)
        print("[XENONBYTE] Same Humanoid:", Controls.humanoid == NewHumanoid)
    end
    ...
    if _G.XENONBYTE_CharacterSystem then
        task.spawn(function()
            task.wait(0.5)
            _G.XENONBYTE_CharacterSystem:RestartAllFeatures()
        end)
    end

    print("")
    print("========================================")
    print("[XENONBYTE] HUMANOID REPLACE + ANTI DEATH COMPLETE")
    print("========================================")
end
